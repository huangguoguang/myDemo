define(['text!module/market/tpl.html'],function(tpl){
	var View2 = Backbone.View.extend({
		...
		showList:function(){
				...
				$.each(w.list,function(v,k){
					if(k.list && k.list.length > 0){
						if(w.type * 1 === 1 && k.type.is_zc === '1'){
							listHtml_4 += '\
								<div class="goods" hash="goods/>'+q+'/'+v+'/1/buy">\
									...
									<ul>\
										...
										<li class="goods_price"><i>￥<s>'+parseFloat(k.list[0].gzp_money)+'</s>.00</i>订货金<span>全款￥<u price='+k.type.code+'></u></span></li>\
										<li class="goods_buy" data="['+q+','+v+',1]" lot="lot_4_'+k.type.code+'"></li>\
									</ul>\
								</div>\
								';
						}
						if(w.type * 1 ===1 ){
							listHtml_1 += '\
								<div class="goods" hash="goods/'+ q+'/'+v+'/0/buy">\
									<div class="goods_img"><img src="'+k.type.img_1+'"></div>\
									<ul>\
										<li class="goods_name">'+ k.type.alias+'</li>\
										<li class="goods_num"><span>月订<i lots="lots_1_'+k.type.code+'">0</i>笔</span><span><u>7天退换</u></span><span><u>包邮</u></span></li>\
										<li class="goods_price"><i>￥<s>'+parseFloat(k.list[0].gzp_money)+'</s>.00</i>订货金<span>全款￥<u price='+k.type.code+'></u></span></li>\
										<li class="goods_buy" data="['+q+','+v+',0]" lot="lot_1_'+k.type.code+'"></li>\
									</ul>\
								</div>\
								';
						}
							...
							listHtml_2 +='\
								<div class="goods" hash="goods/'+q+'/'+v+'/0/buy">\
									...
									<ul>\
										...
										<li class="goods_name">'+k.type.alias+'</li>\
										<li class="goods_price">订货金￥'+ parseFloat(k.list[0].gdsy_money) +'</li>\
	    								<li class="goods_num"><span>全款￥<u price='+ k.list[0].gdsy_number +'></u></span></li>\
	    								<li class="goods_buy" data="['+q+','+v+',0]" lot="lot_2_'+ k.type.code +'"></li>\
									</ul>\
								</div>\
								';
						}
            				...
            				listHtml_3 += '\
            					<div class="goods" hash="goods/'+q+'/'+v+'/0/buy">\
	    							...
	    							<ul>\
	    								...
	    								<li class="goods_price">订货金￥'+ parseFloat(k.list[0].gdsy_money) +'</li>\
	    								<li class="goods_num"><span>全款￥<u price='+ k.list[0].gdsy_number +'></u></span></li>\
	    								<li class="goods_buy" data="['+q+','+v+',0]" lot="lot_3_'+ k.type.code +'"></li>\
	    	        				</ul>\
    	        				</div>\
    	        			';
            			}
			...
			if(listHtml_4 !== ''){
				listHtml_4='<div class="title title2"><div>限时众筹</div></div>\
					<div class="goods_rule goods_rule_4">'+listHtml_4+'</div>';
			}
 		...
 		viewClickBuy:function(e){
 			...
 			var model_index = data[0];
 			var dl_index = data[1];
 			var ul_index = 0;
 			//是否众筹
 			var is_zc = data[2];
 			//是爆品，还是限时众筹，还是抢购
 			var mode_type = appData.itemInfo[model_index].type;
 			var list = appData.itemInfo[model_index].list[dl_index].list;
 			//将单位都置换成件
 			var typeHtml = function(){
 				var html_ ='';
 				$.each(list,function(v,k){
 					if(mode_type === '1'){
 						html_+='<li>'+k.gzp_unit.replace(/kg|桶|m³/g,'件')+'</li>';
 					}
 					if(mode_type==='2' || mode_type==='3'){
						html_+= '<li>' + k.gdsy_unit.replace(/kg|桶|m³/g,"件")+ '</li>';
					}
 				});
 				return html_;
 			};
 			var html='\
 				<div class="pop-buy">\
 					...
 					<div class="main"></div>\
 					<div class="total">合计:<i></i></div>\
 				</div>\
 				';
 			
 			MSG.C(html,
 				  '购物',
 				  {t:'取消',f:function(){MSG.X()}},
 				  {t:(is_zc===1)?'请等待...':'确定',f:function(){submitBuy()}}
 			);
 			if(is_zc===1){
 			...
 			var viewTotal = function(){
 				...
 				ct = setTimeout(function(){
 					...
 					if(mode_type ==='2' || mode_type ==='3'){
 						...
 						log([typeIndex,timeIndex,lotNum,gdsyRule,type,id]);
 			...
 			var viewInfo = function(){
 				...
 				if(mode_type ==='1'){
 					...
 					lot = data.gzp_lot;//限购数量
 					info = '\
 						<li class = "img"><img src="'+itemImage[data.gzp_number]+'"></li>\
 						<li>订货金<s>￥'+parseFloat(data.gzp_money)+'</s></li>\
 				...
 				if(mode_type==='2' || mode_type==='3'){
					...
					info = '\
						...
						<li>订货金<s>￥'+ parseFloat(data.gdsy_money) +'</s></li>\
						<li>仓储费<s></s></li>\
